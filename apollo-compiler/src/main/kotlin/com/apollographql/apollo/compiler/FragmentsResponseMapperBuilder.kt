...
class FragmentsResponseMapperBuilder(
      ...
      CodeBlock.builder()
          .add(fragmentFields
              .fold(CodeBlock.builder()) { builder, field ->
                builder.addStatement("\$T \$N = null", field.type.unwrapOptionalType(withoutAnnotation = true), field)
              }
              .build())
          .add(fragmentFields
  ...
  private fun initFragmentCode(fragmentField: FieldSpec): CodeBlock {
    val fieldClass = fragmentField.type.unwrapOptionalType(withoutAnnotation = true) as ClassName
  ...
  private fun mapperFields(fragments: List<FieldSpec>) =
      fragments
          .map { it.type.unwrapOptionalType(withoutAnnotation = true) as ClassName }
          .map {
